...
/** Specifies the location of the center of the CloudLayer. Sets the "East" and "South" coordinates
   ...
   simulated wind (see AtmosphericConditions::SetWind()).
    ...
    and then set these back to reflect the eyepoint before rendering with Atmosphere::BeginFrame()
    and Atmosphere::EndFrame().
...
/** For infinite cloud layers or cloud layers with "wrapping" enabled ( \sa SetCloudWrapping() \sa SetIsInfinite() ) this
...
/** Returns if this CloudLayer supports the generation of shadow maps. Cumulus cloud decks
   ...
   cloud deck has this capability. Shadow maps are not supported under DirectX9 if the
    "render-offscreen" setting in resources/silverlining.config is enabled, or under DirectX10/11.
    For any renderer, shadow maps only work if the setting cumulus-lighting-quick-and-dirty is
    set to "no" (which it isn't by default.)
...
virtual void WrapClouds();
void   AddCloud(Cloud *cloud) {
    clouds.push_back(cloud);
}
void   LightningNotify(const Vector3& pos);
virtual void ProcessAtmosphericEffects(Sky *s) {
    s;
}
...
void    SetLocalUpVector(const Vector3& up) {
    localUp = up;
}
Vector3 GetLocalUpVector() const;
double ComputeWrapFade(const Cloud *c);
